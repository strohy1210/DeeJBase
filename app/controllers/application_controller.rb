class ApplicationController < ActionController::Base
  ...
  def find_image(resource)
    ...
    @image2 = resource.fb_photo
    @image3 = resource.events.where.not(photo_file_name: nil).first.photo.url if resource.events.where.not(photo_file_name: nil).any?
  end
  ...
  def set_params
    ...
    if params[:genre_id] && params[:genre_id] != 'all'
      ...
      @djs = @genre.djs.is_dj.paginate(page: params[:page], per_page: 36).order('sdcl_followers ASC') if @filter=="scld_asc"
      @djs = @genre.djs.is_dj.paginate(page: params[:page], per_page: 36).order('sdcl_followers DESC') if @filter=="scld_desc"
      @djs = @genre.djs.is_dj.paginate(page: params[:page], per_page: 36).order('rated_at DESC') if @filter=="recent_desc"
    else
      @djs = Dj.is_dj.paginate(page: params[:page], per_page: 36).order('sdcl_followers ASC') if @filter=="scld_asc"
      @djs = Dj.is_dj.paginate(page: params[:page], per_page: 36).order('sdcl_followers DESC') if @filter=="scld_desc"
      @djs = Dj.is_dj.paginate(page: params[:page], per_page: 36).order('rated_at DESC') if @filter=="recent_desc"
    end
  end
  ...
  def set_venue_params
    ...
    if params["category"] && params["category"] != "all" && params[:neighborhood_id] != "all"
      ...
      @venues = Venue.where(neighborhood_id: @neighborhood.id, category: @category).paginate(page: params[:page], per_page: 36).order('priority').order('rated_at DESC')
      if @venues.blank?
        flash.now[:warning] = "No venues matching both the neighborhood and category, so here's the results for the hood."
        @venues = Venue.where(neighborhood_id: @neighborhood.id).paginate(page: params[:page], per_page: 36).order('priority').order('rated_at DESC')
      end
    elsif params["category"] && params["category"] != "all" && params[:neighborhood_id] == "all"
      @venues = Venue.where(category: @category).paginate(page: params[:page], per_page: 36).order('priority').order('rated_at DESC')
    elsif params["category"] == "all" && params[:neighborhood_id] && params[:neighborhood_id] != "all"
      @neighborhood = Neighborhood.find(params[:neighborhood_id])
      @venues = Venue.where(neighborhood_id: @neighborhood.id).paginate(page: params[:page], per_page: 36).order('priority').order('rated_at DESC')
    else
      @venues = Venue.paginate(page: params[:page], per_page: 36).order('priority').order('rated_at DESC')
    end
